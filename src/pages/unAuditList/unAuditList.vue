      ...
      <div class="tableTotal">
        <el-table :data="tableData" show-overflow-tooltip style="width: 100%" stripe border>
          ...
          <el-table-column prop="lastModifyTime" label="最后修改时间" align="center">
            <template slot-scope="scope">
              <p>{{scope.row.lastModifyTime | formatDate1}}</p>
            </template>
          </el-table-column>
          <el-table-column label="操作" align="center" width="100">
            <template slot-scope="scope" class="elseFont">
              <span @click="isPass(scope.row,scope.$index)" class="point">审核</span>
            </template>
          </el-table-column>
        </el-table>
...
import { undwrtList, undwrtCheck } from "@/api";
export default {
  components: {
    FormDialog,
    Gender
  ...
  methods: {
    isPass(item, val) {
      console.log(item, this.currentPage, this.pageSize, val);
      localStorage.setItem("reportDetail", JSON.stringify(item));
      let pageNum = this.pageSize * (this.currentPage - 1) + val;
      console.log(pageNum);
      this.$router.push({
        path: "/unAuditDetail",
