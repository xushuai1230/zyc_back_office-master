let validator = {
  ...
  checkDateNum: function (rule, value, callback) {
    ...
    value = Number(value);
    let reg = /^([12]{0,1}[1-9]|30|31|20|10)$/;
    if (isNaN(value)) {
      callback(new Error('请输入数字值'));
  ...
  passWordeNum: function (rule, value, callback) {
    if (!value && value !== 0) {
      callback();
      return;
    }
    let reg = /^[a-zA-Z][0-9a-zA-Z_]{5,17}/;
    if (!reg.test(value)) {
  ...
  // 校验1-31之间的整数
  checkDateNumZero: function (rule, value, callback) {
    if (!value && value !== 0) {
      callback();
      return;
    }
    value = Number(value);
    let reg = /^([12]{0,1}[1-9]|30|31|20|10|0)$/;
    if (isNaN(value)) {
      callback(new Error('请输入数字值'));
    } else if (!reg.test(value)) {
      callback(new Error('请输入1-31之间的整数'));
  ...
  // 校验1-12之间的整数
  checkMouthNumZero: function (rule, value, callback) {
    if (!value && value !== 0) {
      callback();
      return;
    }
  ...
  // 校验数字大于0(只能是整数)
  checkThanNum: function (rule, value, callback) {
  ...
  // 校验数字大于0(整数小数均可)
  checkFloatIntNum: function (rule, value, callback) {
    if (isNaN(value)) {
      callback(new Error('请输入数字值'));
      return false;
    }
  ...
  checkTelAndphone: function (rule, value, callback) {
    ...
    let reg = /^((1[3|4|5|7|8][0-9]{9})|(0\d{2,3}-\d{7,8}(-\d{1,6})?))$/;
    if (!reg.test(value)) {
  ...
  checkMax15TwoNum: function (rule, value, callback) {
    let reg = /^[0-9]{0,13}(([.]{1}\d{1,2}))?$/;
    if (!reg.test(value)) {
      callback(new Error('请输入整数位最多十三位，最多保留两位小数数字'));
    } else {
      callback();
    }
  },
